 ...
 */
             ...
             const cv::Mat &srcimg)
  {
    ...
    // 调试用，自行替换需要修改的路径
    cv::imwrite("/sdcard/vis.png", img_vis);
  }
                            ...
                            cv::Mat &origin)
  {
    ...
    std::cout <<"_det_predictor\t"<< (double)(clock() - start) / CLOCKS_PER_SEC << std::endl;
    start = clock();
    ...
    std::cout <<"calc_filtered_boxes\t"<< (double)(clock() - start) / CLOCKS_PER_SEC << std::endl;
    start = clock();
    std::vector<OCRPredictResult> aa= infer_rec(filtered_box, origin);

    std::cout <<"infer_rec+cls\t"<< (double)(clock() - start) / CLOCKS_PER_SEC << std::endl;
    start = clock();
    return aa;
  }
                                      ...
                                      const cv::Mat &origin)
  {
    ...
    // cv::imwrite("/sdcard/cbuf_map.jpg",cbuf_map*150);
    cv::Mat bit_map;
    cv::threshold(cbuf_map, bit_map, threshold, maxvalue, cv::THRESH_BINARY);
    // cv::imwrite("/sdcard/bit_map.jpg",bit_map*150);
    ...
    // visual_img(filter_boxes, boxes, origin);
    return filter_boxes;
  }
